 ...
 * Returns contract fields to merge into CompanySkill.metadata, or null if absent.
 */
...
function getSkillContractBadges(metadata: Record<string, unknown> | null): {
  ...
  const skillsLoopVersion = asString(metadata.skillsLoopVersion);
  if (!skillsLoopVersion) {
    return { routingPriority: null, maturity: null, contractVersion: null };
  }
  ...
  return {
    ...
    contractVersion: skillsLoopVersion,
  };
}
    ...
    if (repo && owner && repoName) {
      ...
      return {
        ...
        metadata: {
          ...
          ...(extractSkillContractMetadata(frontmatter) ?? {}),
        },
      };
    }
  }
